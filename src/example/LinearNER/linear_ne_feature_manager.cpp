...
FeatureArray* LinearNEFeatureManager::ExtractHelper(Network *ptr_network, int parent, int *ptr_children,
    ...
    /* transition */
    std::string entity_str = std::to_string(eId);
    /*why the index is 1 ?????? */
    std::string prev_entity_str = std::to_string(child_vev[1]);
    ...
    FeatureArray *ptr_word_features = new FeatureArray(new int(featureID), 1, ptr_features);
    ...
    if(ComParam::USE_HYBRID_NEURAL_FEATURES == NetworkConfig::Feature_Type){ /* neural */
        if(ComType::NeuralType::LSTM == NetworkConfig::Neural_Type){
            ...
            /* noted that the ouput is eID - 1 and it is not eID, why eId - 1 ????? */
            AddNeural(ptr_network,0,parent,children_k_index,ptr_nn_input_pair,eId - 1);
        }
        ...
        ptr_features = new FeatureArray(new int(featureID), 1, ptr_features);
