  ...
  Expression BuildLMGraph(const vector<int>& sent, ComputationGraph& cg, bool apply_dropout) {
    ...
    for (unsigned t = 0; t < slen; ++t) { // h_t = RNN(x_0,...,x_t)
      if (cfsm) { // class-factored softmax
        ...
        Expression u_t = affine_transform({bias, R, h_t});//nan: w*x + b
  ...
  if (TRAIN) {
    ...
    while (!INTERRUPTED) {
      ...
      for (unsigned i = 0; i < report_every_i; ++i) {
        ...
        int param_no = 1;
        for(auto it = storage.params.begin(); it != storage.params.end(); ++it){
            //const ParameterStorage &ptr = (*it);
            std::vector<real> vec = dynet::as_scale_vector((*it)->values, (*it)->owner->get_weight_decay().current_weight_decay());
            int j = 1;
            for(auto itt = vec.begin(); itt != vec.end(); ++itt){
                std::cout << param_no << " th param vector " << j <<"th value is: "<<(*itt)<<std::endl;
                ++j;
            }
            ++param_no;
        }
